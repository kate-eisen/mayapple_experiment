...
```{r}
Sever_2 <- emmip(sur_t1f_bm,   ~ Sever  , type="response",CIs = TRUE)+xlab("")+ylab("Predicted probability")+mdthemes::md_theme_classic()+ggtitle("*Front survival in year t+1*")
...
```{r}
Sever_3 <- emmip(mod_t1laf.bm,   ~ Sever  ,CIs = TRUE)+xlab("")+mdthemes::md_theme_classic()+ggtitle("*Front leaf area in year t+1*")
...
```{r}
Sever_4 <- emmip(sur_t1b_bm,   ~ Sever  ,type="response",CIs = TRUE )+xlab("")+ylab("Predicted probability")+mdthemes::md_theme_classic()+ggtitle("*Back shoot production in year t+1*")
...
```{r}
Sever_5 <- emmip(b_mod_t1la.bm,  ~ Sever  ,CIs = TRUE )+mdthemes::md_theme_classic()+ggtitle("*Back leaf area in year t+1*")
Time_5 <- emmip(b_mod_t1la.bm,  ~ Time  ,CIs = TRUE )+ylab("")+mdthemes::md_theme_classic()
```
...
a<-emmip(s_t1mod1, Sever~Status  ,CIs = TRUE,linearg = list(linetype = NA))+ ylab("Total leaf area")+ 
...
b<-emmip(s_t1mod2, Sever~Status  ,CIs = TRUE, linearg = list(linetype = NA))+ ylab("Average leaf area per system")+ 
...
e <- ggplot(data=mayappleData, aes(x=LfFtot_t1, y=LftotF_t2, color=Sex_t)) +
  geom_point() + geom_smooth(method = "lm", se = FALSE)+theme_classic()+
  ylab("Front leaf area in year t+2") +xlab("Front leaf area in year t+1")+ scale_color_discrete(name="Sexual status in year t")
...
f <- ggplot(data=mayappleData, aes(x=Lf_t, y=LftotB_t2, color=Sex_t)) +
  geom_point() + geom_smooth(method = "lm", se = FALSE)+theme_classic()+
  ylab("Back leaf area in year t+2") +xlab("Leaf area in year t")+ scale_color_discrete(name="Sexual status in year t")

g <- ggplot(data=mayappleData, aes(x=LfBtot_t1, y=LftotB_t2, color=Sex_t)) +
  geom_point() + geom_smooth(method = "lm", se = FALSE)+theme_classic()+
  ylab("Back leaf area in year t+2") +xlab("Back leaf area in year t+1")+ scale_color_discrete(name="Sexual status in year t")
...
a<-emmip(s_t2mod1, Sever~Status  ,CIs = TRUE,linearg = list(linetype = NA))+ ylab("Total leaf area")+ 
  theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+xlab("")+theme(legend.position = "none")+theme(panel.border = element_blank(), axis.line = element_line())
...
b<-emmip(s_t2mod2, Sever~Status  ,CIs = TRUE,linearg = list(linetype = NA))+ ylab("Average leaf area per system")+ 
  theme_bw()+theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank())+scale_y_continuous(limits=c(-250, 1500))+ geom_text(data=counts, aes(x=Status, y=N, label=N, color=Sever))
